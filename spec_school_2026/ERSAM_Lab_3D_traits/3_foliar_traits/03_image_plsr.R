...
setwd("C:/Users/kdahlin/Dropbox/NEON_hsi_lidar/hsi_lidar_ms1/data/")
today <- format(Sys.Date(), "%Y%m%d")
dir.create(paste0("./output_", today))
dir.create(paste0("../figures/figures_", today))
fig.dir <- paste0("../figures/figures_", today, "/")
loc1 <- "SERC"
loc2 <- "HARV"
all <- "SERC_HARV_2017"

# get the file produced in the prev r script
hsi.data1 <- read.csv(paste0(loc1, "_points_to_keep_FINAL_20180726.csv"), 
                      stringsAsFactors = FALSE)

hsi.data2 <- read.csv(paste0(loc2, "_points_to_keep_FINAL_20181121.csv"), 
                      stringsAsFactors = FALSE)

hsi.data <- rbind(hsi.data1, hsi.data2)

hsi.data$pointIDs <- toupper(hsi.data$pointIDs)

names(hsi.data)[1] <- "Sample"

# read in the field plsr data
in.N.data <- read.csv(paste0("./field_data/plsr_N/out_20181021_FINAL",
                             "/PLSR_lab_N_Estimates_FINAL.csv"),
                      stringsAsFactors = FALSE)

in.LMA.data <- read.csv(paste0("./field_data/plsr_LMA/out_20181023",
                               "/PLSR_lab_LMA_Estimates_FINAL.csv"),
                        stringsAsFactors = FALSE)

trait.data <- base::merge(in.N.data[,c(1,5)], 
                          in.LMA.data[,c(1,5)], 
                          by = "Sample")

# read in data loc info -> this is a bit of a mess CLEAN UP!
in.sample.data <- read.csv("ALL_cleaned_data_locs_201807803.csv",
                           stringsAsFactors = FALSE)

in.sample.data <- in.sample.data[,c(2,6:8)]
names(in.sample.data)[1] <- "Sample"
in.sample.data$Sample <- toupper(in.sample.data$Sample)

# read in wavelengths file for plotting
wavelengths <- read.csv("NEON_HSI_wavelengths_20181120.csv",
                        stringsAsFactors = FALSE)
...
# merge dataframes
all.data <- base::merge(trait.data, 
                        in.sample.data, 
                        by = "Sample")

all.data <- base::merge(all.data, 
                        hsi.data, 
                        by = "Sample")

all.data <- all.data[,-c(436,437)]

write.csv(all.data, paste0("./output_", today, "/",
                           all, "_cleaned_data_ALL_", today, ".csv"))
...
clean.data.1.0 <- cbind(all.data[,1:9], all.data[10:435]/10000)
...
for (i in 1:dim(clean.data.1.0)[1]) {
  x <- as.numeric(clean.data.1.0[i,10:435])
  clean.data.smth[i,10:435] <- smth.gaussian(x,
                                             window = 7,
                                             alpha = 1,
                                             tails = FALSE)
  print(i)
...
upper.clean.data <- subset(clean.data.smth, clean.data.1.0$CANP == "U")

# take a look!
x11()
matplot(wavelengths, 
        t(upper.clean.data[10:435]), 
        type = "l", 
        xlab = "Wavelength (nm)",
        ylab = "Reflectance",
        main = "SERC & HARV upper canopy sample refl (11 species, n = 116)")
...
matplot(wavelengths, 
        t(subset(upper.clean.data[10:435], upper.clean.data$Sample == "HARV018")), 
        type = "l", 
        col = "black",
        add = TRUE)
matplot(wavelengths, 
        t(subset(upper.clean.data[10:435], upper.clean.data$Sample == "HARV022")), 
        type = "l", 
        col = "black",
        add = TRUE)
...
clean.data.avg <- as.data.frame(matrix(NA, 
                                       nrow = nrow(all.data), 
                                       ncol = (9+length(wave.names))))
# write the names
names(clean.data.avg) <- c(names(all.data)[1:9], 
                           wave.names)
# keep all of the non spectral info the same
clean.data.avg[,1:9] <- all.data[,1:9]
...
for (k in 1:length(wave.names)) {
  clean.data.avg[,k+9] <- rowMeans(cbind(clean.data.smth[,count.3[k]],
                                         clean.data.smth[,count.3[k]+1]),
                                   na.rm = T)
}

matplot(waves.avg, 
        t(subset(clean.data.avg[,10:222], clean.data.avg$CANP == "U")), 
        type = "l", 
        xlab = "Wavelength (nm)",
        ylab = "Reflectance",
        main = "SERC & HARV upper canopy sample refl (10 nm) (11 species, n = 116)"
)
...
keep.waves <- as.numeric(which.keep.cols[10:length(which.keep.cols)]) - 9
clean.waves <- waves.avg[keep.waves]

# take a look
x11()
matplot(clean.waves, t(clean.data[10:ncol(clean.data)]), type = "l")

# write this out to a file for safe keeping
write.csv(clean.data, paste0("./output_", today, "/",
                             all, "_averaged_data_ALL_", today, ".csv"))
...
upper.BN.data <- subset(BN.data, BN.data$CANP == "U")

#upper.data <- subset(clean.data.0.1, clean.data.0.1$CANP == "U")

# take another look
matplot(clean.waves, 
        t(upper.BN.data[,10:ncol(upper.BN.data)]), 
        type = "l",
        xlab = "Wavelength (nm)",
        ylab = "Reflectance (BN)",
        main = "SERC & HARV upper canopy Brightness Normalized refl (10 nm) (11 species, n = 116)")
...
species <- unique(upper.BN.data$SP)

upper.train <- NA
upper.test <- NA

for (j in 1:length(species)) {
  upper.sp <- subset(upper.data, upper.data$SP == species[j])
  n <- dim(upper.sp)[1]
  rand <- runif(n)
  rand.sort <- sort(rand)
...
write.csv(upper.train, paste0(all,"_training_data_HSI_avg_PLSR_", today,".csv"),
          row.names = FALSE)

write.csv(upper.test, paste0(all,"_test_data_HSI_avg_PLSR_", today,".csv"),
          row.names = FALSE)
...
plsr.spectra <- as.matrix(upper.BN.data[,10:ncol(upper.BN.data)])
plsr.dataset <- data.frame(PLSR_LMA = (upper.BN.data$out.lma), 
                           spectra = I(plsr.spectra))

test.spectra <- as.matrix(upper.test[,10:ncol(upper.test)])
test.dataset <- data.frame(PLSR_N = upper.test$out.N, spectra = I(test.spectra))
...
plot(clean.waves, 
     seq(from = -0.7, to = 0.7, length = length(clean.waves)),
     type = "n",
     xlab = "Wavelength (nm)", 
     ylab = "Correlation / Refl (scaled)")

matplot(clean.waves, t(plsr.spectra), 
        type = "l", 
        lty = 1, 
        ylab = "Reflectance (%)", 
        xlab = "Wavelength (nm)",
...
par(mar = c(4,4,3,3))
plot(upper.BN.data$out.lma, upper.BN.data$out.lma, 
     type = "n",
     xlab = "Modelled LMA (g/m2)",
     ylab = "Measured LMA (g/m2)",
     main = "PLSR Modelled Top of Canopy LMA")
abline(lm(upper.BN.data$out.lma ~ fit1), lwd = 2)
abline(0, 1, col = "red", lwd = 2, lty = 2)
graphics::text(fit1, 
               upper.BN.data$out.lma, 
               labels = upper.BN.data$SP, 
               cex = 0.6)

dev.off()

summary(lm(upper.BN.data$out.lma ~ (fit1)))




# run to here then apply to all point data
plsr.predicted <- predict(plsr.out, 
                          ncomp = nComps, 
                          newdata = as.matrix(BN.data[,10:ncol(BN.data)]))
