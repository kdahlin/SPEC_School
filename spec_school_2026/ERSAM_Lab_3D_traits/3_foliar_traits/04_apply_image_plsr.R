...
# export folder
work.dir <- "Y:/meicheng/trait_mapping"

hpcc.data.dir <- "X:/Dissertation/TraitMapping"

Coefficient_folder <- sprintf("%s/Data/Coefficients/", hpcc.data.dir)

tiff.dir <- "D:/Meicheng/Dissertation/trait_mapping/directional/SERC"
...
wavelength_file <- "Y:/meicheng/NEON_hsi_wl_update.csv"
...
site_colID <- which(names(wavelength_matrix) == sprintf("%s_%s", siteID, siteYear))
...
footprint_raster_filename <- sprintf("%s_%s_HSI_mosaic_raw.tif", siteID, siteYear)
...
plot(LMA$avg.layer, zlim = c(LMA_lower, LMA_upper), main = "LMA trait map (average across 200 PLSR models)")
...
plot(Chl$avg.layer, zlim = c(Chl_lower, Chl_upper), main = "Chlorophyll trait map (average across 200 PLSR models)")
...
plot(Car$avg.layer, zlim = c(Car_lower, Car_upper), main = "Carotenoid trait map (average across 200 PLSR models)")
...
plot(EWT$avg.layer, zlim = c(EWT_lower, EWT_upper), main = "Equivalent Water Thickness (EWT) trait map (average across 200 PLSR models)")
...
LMA.cv.filename <- sprintf("%s/%S_LMA_cv_footprint_%d.tif", work.dir, siteID, radius)
Chl.cv.filename <- sprintf("%s/%S_Chl_cv_footprint_%d.tif", work.dir, siteID, radius)
Car.cv.filename <- sprintf("%s/%S_Car_cv_footprint_%d.tif", work.dir, siteID, radius)
EWT.cv.filename <- sprintf("%s/%S_EWT_cv_footprint_%d.tif", work.dir, siteID, radius)
