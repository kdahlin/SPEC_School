# Interactive function for processing ERSAM lab hemispherical photos. 
#
...
#' @param channel Integer. The band number channel in RGB images. Default '3'
#' for blue. Native function hemispheR::import_fisheye.
#' @param stretch Logical. Applies linear stretch to the image to enhance contrast.
#' Native function hemispheR::import_fisheye.
#' @param gamma Integer. The gamma of the image, which is then back corrected. The default
#' is 2.2, which is typical for most image formats. Native function hemispheR::import_fisheye.
      ...
      while (!satisfied) {
        ...
        if (yes_display == TRUE) {
          ...
          if (tolower(response) == "yes") {
            ...
            adjust_gamma <- readline(prompt = paste("Would you like to adjust the gamma?",
                                                    "(yes/no or Enter to keep current): "))
            
            if (tolower(adjust_gamma) == "yes") {
              new_gamma <- readline(prompt = paste("Enter new gamma value 
            ...
            # Ask user if they want to apply linear stretch
            stretch_input <- readline(prompt = paste("Toggle stretch?", 
