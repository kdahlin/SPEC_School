...
#' @return A named list containing filled voxel ratio and porosity ratio rasters.
#' @export

canopy.porosity.ratio <- function(lad.array, laz.array, ht.cut, epsg.code) {
  ...
  if (ht.cut < n_z) {
    ...
    for (r in 1:n_y) {
      for (c in 1:n_x) {
        ...
        # add 1 to account for the NA layer
        ...
        total <- filled + porosity
        
        if (total == 0){
          filled.mat[r,c] <- NA_real_
          porosity.mat[r,c] <- NA_real_
        } else { 
          filled.ratio <- round(filled / total, digits = 4)
          porosity.ratio <- round(porosity / total, digits = 4)
  ...
  # list matrices 
  mat.list <- list (
    filled.ratio.raster = filled.mat,
    empty.ratio.raster = porosity.mat
  )
